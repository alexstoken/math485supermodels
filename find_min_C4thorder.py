...
def get_lastest_C(fname):
    ...
    """
    C_headers = ['F','cx12', 'cx13', 'cx14','cx21', 'cx23','cx24', 'cx31', 'cx32','cx34','cx41','cx42','cx43','cy12', 'cy13', 'cy14','cy21','cy23', 'cy24', 'cy31', 'cy32','cy34','cy41','cy42','cy43','cz12', 'cz13','cz14', 'cz21', 'cz23', 'cz24','cz31','cz32','cz34', 'cz41','cz42','cz43']
...
def cost_fn(C, K =200, gamma= 0.4, delta =1, fname= None, data = None, verbose = True):
    ...
    #set x_o,y_o, x_o
    #[TODO] Check which needs to be changed to
    x_init = [x_o[0,i] for i in range(4) for i in range(3)]
    ...
    for i in range(K):
        ...
        #[TODO] Find another perturbed value
...
def conn_lorentz(x_vect, t0,C, sig = [10,10,10,10], rho = [28,28,28,28], beta = [8 /3,8/3,8/3,8/3]):
    """
    Parameters:
    ...
    """
    x1,y1,z1, x2, y2, z2, x3, y3, z3, x4, y4, z4 = x_vect

    cx12, cx13, cx13,cx23, cx21, cx24, cx31, cx32, cx34, cx41, cx42, cx43, cy12, cy13, cy14, cy23, cy21, cy24, cy31, cy32,cy34, cy41, cy42, cy43, cz12, cz13, cz14, cz23, cz24, cz21, cz31, cz32, cz34, cz41, cz42, cz43,= C

    C_x1 = cx12 * (x2 - x1) + cx13 * (x3-x1) + cx14 * (x4-x1)
    C_x2 = cx21 * (x2 - x1) + cx23 * (x3-x2) + cx24 * (x4-x2)
    C_x3 = cx31 * (x1 - x3) + cx32 * (x2-x3) + cx34 * (x4-x3)
    C_x4 = cx41 * (x1 - x4) + cx42 * (x2-x4) + cx43 * (x3-x4)


    C_y1 = cy12 * (y2 - y1) + cy13 * (y3-y1) + cy14 * (y4-y1)
    C_y2 = cy21 * (y2 - y1) + cy23 * (y3-y2) + cy24 * (y4-y2)
    C_y3 = cy31 * (y1 - y3) + cy32 * (y2-y3) + cy34 * (y4-y3)
    C_y4 = cy41 * (y1 - y4) + cy42 * (y2-y4) + cy43 * (y3-y4)

    C_z1 = cz12 * (z2 - z1) + cz13 * (z3-z1) + cz14 * (z4-z1)
    C_z2 = cz21 * (z2 - z1) + cz23 * (z3-z2) + cz24 * (z4-z2)
    C_z3 = cz31 * (z1 - z3) + cz32 * (z2-z3) + cz34 * (z4-z3)
    C_z4 = cz41 * (z1 - z4) + cz42 * (z2-z4) + cz43 * (z3-z4)

    sig1,sig2,sig3,sig4 = sig
    ...
    dx4 = sig4 * (y4-x4) + C_x4
    dy4 = x4 * (rho3 - z3) - y4 + C_y4
    dz4 = x4 * y4 - beta4 *z4 +C_z4
...
def supermodel(sol):
    ...
    """
    x_s = 1/4 * (sol[:,0:4] + sol[:,4:8] + sol[:,8:12])
    return x_s
...
def run_sm_optimization(last_fname = None, verbose = True, k = 10):
    ...
    x_o = load_lorentz_data('TrueValues_SuperModel.xlsx')
    x_init = [x_o[0,i] for i in range(4) for i in range(4)]
    C_init = np.zeros(18)
    t = np.arange(0,20,0.01)
    ...
    truth = integrate.odeint(conn_lorentz, x_init, t, args=(C_init,[10,10,10,10], [28,28,28,28],[8/3, 8/3,8/3, 8/3]))
    data = truth[:, 0:4]
...
def animate_l63(sm1_file, sm2_file, output_fname, k, plt_labels = ['SM1', 'SM2'], t_max = 1000, frame_num = 5000):
    ...
    x_o = load_lorentz_data('TrueValues_SuperModel.xlsx')
    x_init = [x_o[0,i] for i in range(3) for i in range(4)]

    #[TODO] add 4th order to animation
